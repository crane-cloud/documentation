...
2\. The deployment modal will popup for you to fill and deploy an application. The Name field will be for your application name in the example it is "nginxapp", the number of replicas even when not selects defaults to 1. The image uri, is the docker hub image link, the first part is the docker hub username/image name and as in the example below "nginxdemos" is the dockerhub username and "hello" is the image name.
